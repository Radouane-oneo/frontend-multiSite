...
function scriptSearch($ChainSearch, $ChainRelace){
	...
	mysql_select_db ($db)
	or die ("la base de donnée: $db n'existe pas".mysql_error());
	$liste = "show tables";
					...
					if (count($tab) > 0){
						foreach($tab as $cle=>$valeur){     
							if(preg_match('#'.$ChainSearch.'#', $valeur)){
								if (is_string($cle)){
									$change = "UPDATE ".$List_rep[0]." SET ".$cle." = REPLACE(".$cle.",'".$ChainSearch."','".$ChainRelace."')";
...
scriptSearch("images.printconcept.com","images.flyer.eu");
